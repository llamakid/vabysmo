...
(function($){
    ...
    var move = function($scroller, settings) {
     ...
     //set velocity towards target position
       if(settings.target>=0) {
            if(scroller.scrollTop > settings.target) {
               settings.velocityY=(-settings.targetVelocity);
           } else {
              settings.velocityY=settings.targetVelocity;
         }
       }
    
        console.log("target: "+settings.target+" velocity: "+settings.velocityY);
    ...
    if(5>=Math.abs(scroller.scrollTop - settings.target)) {
    settings.target=-1;
    ...
    //        ,   target: function(settings, options){
    //          var $this = $(this);
    //              settings = $.extend(settings, options);
    //          if (settings) {
    //              settings.decelerate = false;
